...
class Transaction extends Model
{
    ...
    */
    ...
    {
        return $this->belongsTo(TransactionWorkflowStep::class, 'current_workflow_step_id');
    }
    ...
    {
        return $this->hasMany(TransactionWorkflowStep::class)->orderBy('step_order');
    }

    public function histories(): HasMany
    {
        return $this->hasMany(TransactionHistory::class)->orderBy('created_at');
    }
    ...
    */
    ...
    public function scopeFilter(Builder $query, array $filters): Builder
    {
        return $query
            ...
            ->when($filters['transaction_type_id'] ?? null, fn ($q, $id) => $q->where('transaction_type_id', $id))
            ->when($filters['source_department_id'] ?? null, fn ($q, $id) => $q->where('source_department_id', $id))
            ->when($filters['destination_department_id'] ?? null, fn ($q, $id) => $q->where('destination_department_id', $id))
            ->when($filters['current_department_id'] ?? null, fn ($q, $id) => $q->where('current_department_id', $id))
            ...
            ->when($filters['sort'] ?? null, fn ($q, $sort) => $this->applySort($q, $sort))
            ->when(! isset($filters['sort']), fn ($q) => $q->orderBy('created_at', 'desc'));
    }
