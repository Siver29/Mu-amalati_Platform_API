...
function EditTransactionType() {
  ...
  // --------------------------------------------------
  // Load transaction type + departments
  // --------------------------------------------------
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.error(
          'Edit transaction type load error:',
          error.response?.data ||
            error
        )
        ...
        setPageError(
          error.response?.data
            ?.message ||
            'Unable to load transaction type data.'
        )
      } finally {
        setLoading(false)
      }
    ...
    try {
      ...
      if (backendErrors) {
        ...
        setPageError(
          error.response?.data
            ?.message ||
            'Unable to update transaction type.'
        )
      }
    } finally {
      setSaving(false)
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-8 dark:bg-gray-950">
      ...
      <div className="mt-6">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Update the transaction type configuration.
        </p>
